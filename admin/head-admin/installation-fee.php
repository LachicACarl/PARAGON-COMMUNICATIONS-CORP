...
<head>
    ...
    <title>Installation Fee Management - Head Admin</title>
        ...
        <main class="h-screen overflow-auto">
            ...
            <div class="bg-white border-b sticky top-0 z-10">
                <div class="px-6 py-4 flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Installation Fee Management</h1>
                        <p class="text-sm text-gray-600 mt-1">Configure installation fee amounts and descriptions</p>
                    </div>
                    <button class="btn-primary" onclick="openModal()">
                        <span class="material-icons">add</span> Add Fee
                    </button>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="bg-white rounded-lg shadow">
                    <div class="p-4 border-b">
                        <input type="text" id="searchInput" placeholder="Search fees..." class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">Fee Name</th>
                            ...
                            <tbody id="feeTable" class="divide-y">
...
<div class="modal" id="feeModal">
    <div class="modal-content">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-900" id="modalTitle">Add Installation Fee</h2>
            <button class="text-gray-400 hover:text-gray-600 text-2xl" onclick="closeModal()">✕</button>
        </div>
        <form id="feeForm" class="p-6 space-y-4">
            <input type="hidden" id="feeId">
            
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Fee Name *</label>
                <input type="text" id="feeName" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required placeholder="e.g., Standard Installation">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Amount *</label>
                <input type="number" id="feeAmount" step="0.01" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required placeholder="0.00">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Description</label>
                <textarea id="feeDescription" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Optional description"></textarea>
            </div>

            <div class="flex gap-2 pt-4">
                <button type="submit" class="flex-1 btn-primary justify-center"><span class="material-icons">save</span> Save Fee</button>
    ...
    <div class="modal-content">
        <div class="p-6">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div><h3 class="text-lg font-bold text-gray-900">Delete Fee?</h3><p class="text-sm text-gray-600">This action cannot be undone.</p></div>
            </div>
            <div class="flex gap-2 pt-4">
...
function loadData(page = 1) {
    ...
    fetch(`${BASE_URL}api/fetch-installation-fee.php?page=${page}&search=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
            if (!data.success) { showToast('Error loading fees', 'error'); return; }
            currentPage = page;
            const tbody = document.getElementById('feeTable');
            tbody.innerHTML = data.data.map(fee => `
                <tr class="table-row">
                    <td class="table-cell font-mono text-gray-600">${fee.id}</td>
                    <td class="table-cell font-semibold text-gray-900">${fee.fee_name}</td>
                    <td class="table-cell font-semibold text-green-600">₱${parseFloat(fee.amount).toFixed(2)}</td>
                    <td class="table-cell text-gray-600">${fee.description ? fee.description.substring(0, 50) : '—'}</td>
                    <td class="table-cell text-gray-600">${new Date(fee.created_at).toLocaleDateString('en-US', {year: 'numeric', month: 'short', day: 'numeric'})}</td>
                    <td class="table-cell text-center">
                        <button class="text-blue-600 hover:text-blue-800 text-sm font-medium" onclick="editFee(${fee.id}, '${fee.fee_name.replace(/'/g, "\\'")}', ${fee.amount}, '${(fee.description || '').replace(/'/g, "\\'").replace(/\n/g, "\\n")}')">✏️ Edit</button>
                        <button class="text-red-600 hover:text-red-800 text-sm font-medium ml-2" onclick="openDeleteModal(${fee.id})">🗑️ Delete</button>
                    </td>
                </tr>
            `).join('');
...
function openModal() {
    editingId = null;
    document.getElementById('modalTitle').textContent = 'Add Installation Fee';
    document.getElementById('feeForm').reset();
    document.getElementById('feeId').value = '';
    document.getElementById('feeModal').classList.add('show');
}

function editFee(id, name, amount, desc) {
    editingId = id;
    document.getElementById('modalTitle').textContent = 'Edit Installation Fee';
    ...
    document.getElementById('feeDescription').value = desc;
    document.getElementById('feeModal').classList.add('show');
}

function closeModal() { document.getElementById('feeModal').classList.remove('show'); }
...
document.getElementById('feeForm').addEventListener('submit', function(e) {
    ...
    formData.append('fee_name', document.getElementById('feeName').value);
    formData.append('amount', document.getElementById('feeAmount').value);
    formData.append('description', document.getElementById('feeDescription').value);
    const url = editingId ? `${BASE_URL}api/update-installation-fee.php` : `${BASE_URL}api/add-installation-fee.php`;
    if (editingId) formData.append('fee_id', editingId);
    fetch(url, { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
            if (data.success) { showToast(data.message, 'success'); closeModal(); loadData(currentPage); }
...
function confirmDelete() {
    ...
    fetch(`${BASE_URL}api/delete-installation-fee.php`, { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
            if (data.success) { showToast(data.message, 'success'); closeDeleteModal(); loadData(currentPage); }
            else { showToast(data.message || 'Error', 'error'); }
        })
        .catch(err => showToast('Request failed', 'error'));
}
