...
<head>
    ...
    <title>Pull Out Remarks Management - Paragon Head Admin</title>
        ...
        <main class="h-screen overflow-auto">
            <div class="bg-white border-b sticky top-0 z-10">
                <div class="px-6 py-4 flex justify-between items-center">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Pull Out Remarks Management</h1>
                        <p class="text-sm text-gray-600 mt-1">Create and manage pull-out remark templates</p>
                    </div>
                    <button class="btn-primary" onclick="openModal()">
                        <span class="material-icons">add</span> Add Remark
                    </button>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="bg-white rounded-lg shadow">
                    <div class="p-4 border-b">
                        <input type="text" id="searchInput" placeholder="Search remarks..." class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase">Remark Text</th>
...
<div class="modal" id="dataModal">
    <div class="modal-content">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-900" id="modalTitle">Add Remark</h2>
        ...
        <form id="dataForm" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Remark Text *</label>
                <input type="text" id="dataName" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required placeholder="Enter remark text">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Description</label>
                <textarea id="dataDescription" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Optional description"></textarea>
            </div>
    ...
    <div class="modal-content">
        <div class="p-6">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Delete Remark?</h3>
                    <p class="text-sm text-gray-600">This action cannot be undone.</p>
                </div>
            </div>
            <div class="flex gap-2 pt-4">
                <button onclick="confirmDelete()" class="flex-1 btn-danger bg-red-600 justify-center"><span class="material-icons">delete</span> Delete</button>
...
const TABLE_NAME = 'pull_out_remarks';
let currentPage = 1;
let editingId = null;
let deleteId = null;
        ...
        .then(data => {
            ...
            tbody.innerHTML = data.data.map(item => `
                <tr class="table-row">
                    ...
                    <td class="table-cell font-semibold text-gray-900">${item.remark_text}</td>
                    ...
                    <td class="table-cell text-center">
                        <button class="text-blue-600 hover:text-blue-800 text-sm font-medium" onclick="editItem(${item.id}, '${item.remark_text.replace(/'/g, "\\'")}', '${(item.description || '').replace(/'/g, "\\'").replace(/\n/g, "\\n")}')">✏️ Edit</button>
...
function openModal() {
    editingId = null;
    document.getElementById('modalTitle').textContent = 'Add Remark';
...
function editItem(id, name, desc) {
    editingId = id;
    document.getElementById('modalTitle').textContent = 'Edit Remark';
    document.getElementById('dataId').value = id;
    document.getElementById('dataName').value = name;
    document.getElementById('dataDescription').value = desc;
    document.getElementById('dataModal').classList.add('show');
}
...
document.getElementById('dataForm').addEventListener('submit', function(e) {
    ...
    formData.append('remark_text', document.getElementById('dataName').value);
