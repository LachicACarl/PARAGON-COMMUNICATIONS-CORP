...
<head>
  ...
  <title>Call Out Status | Paragon</title>
...
<body>
  ...
  <aside class="sidebar">
    ...
    <main class="main-content">
      <header class="header">
        <h1>Call Out Status Management</h1>
        <div class="user-info">
      ...
      <section class="content">
        <div class="card">
          ...
          <div class="search-bar">
            ...
            <input type="text" id="searchInput" placeholder="Search call out status..." onkeyup="performSearch()">
          </div>
  ...
  <script>
    ...
    async function loadData() {
      try {
        const response = await fetch('../api/fetch-call-out-status.php');
    ...
    function performSearch() {
      ...
      filteredData = allData.filter(item => 
        item.status_name.toLowerCase().includes(searchTerm) ||
    ...
    async function submitAdd(event) {
      ...
      try {
        const response = await fetch('../api/add-call-out-status.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Call out status added successfully!');
          closeAddModal();
          loadData();
        } else {
        ...
        alert('Error adding call out status');
      }
    }

    async function deleteItem(id) {
      if (!confirm('Are you sure you want to delete this call out status?')) return;

      try {
        const formData = new FormData();
        formData.append('id', id);

        const response = await fetch('../api/delete-call-out-status.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Call out status deleted successfully!');
          loadData();
        } else {
          alert('Error: ' + result.message);
        }
      } catch (error) {
        alert('Error deleting call out status');
      }
    }
