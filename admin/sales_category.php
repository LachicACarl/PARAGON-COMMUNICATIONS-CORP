...
<head>
  ...
  <title>Sales Category | Paragon</title>
...
<body>
  ...
  <aside class="sidebar">
    ...
    <main class="main-content">
      <header class="header">
        <h1>Sales Category Management</h1>
        <div class="user-info">
      ...
      <section class="content">
        <div class="card">
          ...
          <div class="search-bar">
            ...
            <input type="text" id="searchInput" placeholder="Search categories..." onkeyup="performSearch()">
          </div>
  ...
  <script>
    ...
    async function loadData() {
      try {
        const response = await fetch('../api/fetch-sales-category.php');
    ...
    async function submitAdd(event) {
      ...
      try {
        const response = await fetch('../api/add-sales-category.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Sales category added successfully!');
          closeAddModal();
          loadData();
        } else {
        ...
        alert('Error adding sales category');
      }
    }

    async function deleteItem(id) {
      if (!confirm('Are you sure you want to delete this sales category?')) return;

      try {
        const formData = new FormData();
        formData.append('id', id);

        const response = await fetch('../api/delete-sales-category.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Sales category deleted successfully!');
          loadData();
        } else {
          alert('Error: ' + result.message);
        }
      } catch (error) {
        alert('Error deleting sales category');
      }
    }
