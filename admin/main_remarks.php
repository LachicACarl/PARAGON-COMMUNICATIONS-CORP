...
<head>
  ...
  <title>Main Remarks | Paragon</title>
...
<body>
      ...
      <section class="content">
        <div class="card">
          ...
          <div class="search-bar">
            ...
            <input type="text" id="searchInput" placeholder="Search main remarks..." onkeyup="performSearch()">
          </div>
  ...
  <div id="addModal" class="modal">
    <div class="modal-content">
      <div class="modal-header">
        <h2>Add Main Remark</h2>
        <span class="close" onclick="closeAddModal()">&times;</span>
      </div>
      <form id="addForm" onsubmit="submitAdd(event)">
        <div class="form-group">
          <label for="remark_name">Remark Name *</label>
          <input type="text" id="remark_name" name="remark_name" required>
        </div>
        <div class="form-group">
  ...
  <script>
    ...
    async function loadData() {
      try {
        const response = await fetch('../api/fetch-main-remarks.php');
        ...
        if (result.success) {
          ...
          document.getElementById('tableBody').innerHTML = `<tr><td colspan="4" class="error">${result.message}</td></tr>`;
        }
      } catch (error) {
        document.getElementById('tableBody').innerHTML = '<tr><td colspan="4" class="error">Error loading data</td></tr>';
      }
    }
    ...
    function performSearch() {
      ...
      filteredData = allData.filter(item => 
        item.remark_name.toLowerCase().includes(searchTerm) ||
    ...
    async function submitAdd(event) {
      ...
      try {
        const response = await fetch('../api/add-main-remarks.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Main remark added successfully!');
          closeAddModal();
          loadData();
        } else {
        ...
        alert('Error adding main remark');
      }
    }

    async function deleteItem(id) {
      if (!confirm('Are you sure you want to delete this main remark?')) return;

      try {
        const formData = new FormData();
        formData.append('id', id);

        const response = await fetch('../api/delete-main-remarks.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Main remark deleted successfully!');
          loadData();
        } else {
          alert('Error: ' + result.message);
        }
      } catch (error) {
        alert('Error deleting main remark');
      }
    }
