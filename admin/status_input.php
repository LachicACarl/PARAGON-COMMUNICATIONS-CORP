...
<head>
  ...
  <title>Status Input | Paragon</title>
...
<body>
      ...
      <section class="content">
        <div class="card">
          ...
          <div class="search-bar">
            ...
            <input type="text" id="searchInput" placeholder="Search status inputs..." onkeyup="performSearch()">
          </div>
  ...
  <script>
    ...
    async function loadData() {
      try {
        const response = await fetch('../api/fetch-status-input-channel.php');
    ...
    async function submitAdd(event) {
      ...
      try {
        const response = await fetch('../api/add-status-input-channel.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Status input added successfully!');
          closeAddModal();
          loadData();
        } else {
        ...
        alert('Error adding status input');
      }
    }

    async function deleteItem(id) {
      if (!confirm('Are you sure you want to delete this status input?')) return;

      try {
        const formData = new FormData();
        formData.append('id', id);

        const response = await fetch('../api/delete-status-input-channel.php', {
          method: 'POST',
          body: formData
        });
        const result = await response.json();
        ...
        if (result.success) {
          alert('Status input deleted successfully!');
          loadData();
        } else {
          alert('Error: ' + result.message);
        }
      } catch (error) {
        alert('Error deleting status input');
      }
    }
